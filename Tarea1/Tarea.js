...
/*Añadi un pequeño sistema de persistencia de datos, 
para que el programa pueda guardar los datos ingresados por el usuario en un archivo de texto, 
y luego leerlos en ejecuciones posteriores para mostrar el inventario sin necesidad de ingresar los datos nuevamente, 
a menos que se elimine el archivo o se modifique su contenido.
*/

import promptSync from 'prompt-sync'; // Importamos la función promptSync para poder solicitar datos al usuario en la consola.
import fs from 'node:fs'; // Importamos el módulo fs para poder escribir en un archivo
...
let inventario = { producto1: {}, producto2: {} }; // Inicializamos el objeto inventario con productos vacíos para evitar errores si el archivo no existe.

if (fs.existsSync(archivo)) { 
    const datos = fs.readFileSync(archivo, 'utf-8'); // Si el archivo existe, lo leemos y cargamos su contenido

    inventario = JSON.parse(datos); // Convertimos el contenido a un objeto JavaScript y lo asignamos a la variable inventario
}
...
// Solicitamos los datos del primer producto al usuario
...
// Solicitamos los datos del segundo producto al usuario
...
inventario = { producto1, producto2 }; // Actualizamos el objeto inventario
}
